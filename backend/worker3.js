...
class GlobalKeyPool {
  ...
  getAvailableKey(scraperId, preferredIndices = []) {
    ...
    if (availableKeys.length > 0) {
      ...
      this.systemLogger.debug(`🔑 Scraper ${scraperId} using fallback key ${bestKey.index + 1}`);
    ...
    if (status && status.assignedTo === scraperId) {
      ...
      if (quotaExceeded) {
        ...
        setTimeout(() => {
          ...
          this.systemLogger.info(`🔄 Key ${index + 1} quota reset after timeout`);
        }, 60 * 60 * 1000);
      }
  ...
  checkAndResetQuota() {
    ...
    for (let i = 0; i < this.allKeys.length; i++) {
      ...
      if (status.quotaExceeded) {
        ...
        if (status.lastUsed && (now - status.lastUsed.getTime()) > 60 * 60 * 1000) {
          status.quotaExceeded = false;
...
class ScraperManager {
  ...
  async initialize() {
    // Load all API keys from environment (supporting up to 50 keys)
    this.apiKeys = [
  ...
  async createKeyGroups() {
    ...
    this.systemLogger.debug(`Created ${this.keyGroups.length} preferred key groups`);
  }
  ...
  async loadOrCreateScrapers() {
    ...
    if (existingScrapers.length > 0) {
      ...
      this.systemLogger.info(`📋 Loaded ${existingScrapers.length} existing scrapers`);
    } else {
      ...
      this.systemLogger.info(`🆕 Created ${this.keyGroups.length} new scrapers`);
    }
  }
  ...
  async createScraper(name, preferredKeyIndices) {
    ...
    await this.systemLogger.success(`✅ Created new scraper: ${name} with ${preferredKeyIndices.length} preferred keys`);
  ...
  startAssignmentLoop() {
    setInterval(async () => {
      try {
        ...
        await this.systemLogger.error('Error in task assignment loop', { error: error.message });
      }
  ...
  startHealthCheckLoop() {
    setInterval(async () => {
      try {
        ...
        await this.systemLogger.error('Error in health check loop', { error: error.message });
      }
    }, 30000);
  }
  ...
  async handleTaskFailure(taskId, scraperId, error) {
    ...
    if (task.retryCount < 3) {
      ...
      await this.systemLogger.info(`🔄 Task ${taskId} will be retried (attempt ${task.retryCount + 1})`);
    }
...
class ScraperWorker {
  ...
  async getKey() {
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      ...
      if (keyInfo) {
        ...
        await this.logger.debug(`🔑 Got key ${keyInfo.index + 1}`, {
  ...
  releaseKey(quotaExceeded = false, failed = false) {
    if (this.currentKeyIndex !== null) {
      ...
      if (quotaExceeded) {
        this.logger.warning(`⚠️ Key ${this.currentKeyIndex + 1} quota exceeded`);
      }
  ...
  async processTask(task) {
    try {
      await this.logger.info(`🚀 Starting task: ${task._id}`, { 
  ...
  async scrapeChannels(task) {
    ...
    await this.logger.info('🚀 Starting scrape - SIRF EMAILS WALE CHANNELS SAVE HONGE', { 
        ...
        try {
          ...
          for (const item of searchResponse.data.items) {
            ...
            if (result.saved) {
              ...
              if (includeRelated && result.channel?.subscriberCount >= minSubscribers) {
                channelQueue.push({
                  channelId,
                  depth: 0,
                  sourceType: 'search'
          ...
          await this.logger.error(`Error searching keyword "${keyword}"`, { error: error.message }, taskId);
    ...
    if (includeRelated && channelQueue.length > 0) {
      await this.logger.info('🔗 Discovering related channels', { 
      ...
      while (queueIndex < channelQueue.length && savedChannels < maxResults) {
        ...
        try {
          ...
          await this.logger.error('Error discovering related channels', { 
  ...
  async enrichKeywords(baseKeywords) {
    ...
    for (const keyword of baseKeywords) {
      ...
      try {
        ...
        await this.logger.error('Error enriching keyword', { keyword, error: error.message });
      }
    }
  ...
  async processChannel(channelData, sourceInfo = {}, taskId = null) {
    ...
    if (existing) {
      if (existing.emails && existing.emails.length > 0) {
        ...
        if (newEmails.length > 0) {
          ...
          await this.logger.debug(`Updated existing channel with new emails`, {
            channelId,
    ...
    try {
      ...
      if (emails.length === 0) {
        await this.logger.debug(`⏭️ Skipping - no emails found`, {
          channelId,
          title: snippet.title
        }, taskId, channelId);
      ...
      const bioLinks = socialLinks.filter(s => 
        s.url.includes('linktr.ee') || s.url.includes('bio.link')
      );
  ...
  extractEmails(text) {
    ...
    const emailPatterns = [
      ...
      /\b(info|contact|support|sales|hello|help|business|marketing|media|press|admin|enquiries|office|team|careers|jobs|hr|partners|sponsorship|collab|collaboration|partnership|sponsor|advertising|ads|pr|inquiry|bookings|management|director|ceo|founder|owner|manager)[@\s@]+[A-Za-z0-9.-]+\.[A-Z|a-z]{2,}\b/gi
    ];
    ...
    for (const pattern of emailPatterns) {
      ...
      matches.forEach(email => {
        let cleaned = email.replace(/\s+/g, '').toLowerCase().trim();
        ...
        if (cleaned.includes('@') && cleaned.includes('.')) {
          ...
          if (parts.length === 2) {
            ...
            if (domain.includes('.') && localPart.length >= 1 && localPart.length <= 64) {
  ...
  try {
    ...
    const task = new Queue({
      ...
      data: { 
        ...
        enrichKeywords,
        saveOnlyWithEmails: true // Sirf emails wale save
      }
    });
    ...
    res.json({ 
      message: 'Task queued successfully - SIRF EMAILS WALE CHANNELS SAVE HONGE', 
      taskId: task._id
    });
  } catch (error) {
...
app.get('/api/channels', async (req, res) => {
  try {
    ...
    const query = { hasEmails: true }; // Sirf emails wale
...
server.listen(PORT, async () => {
  ...
  console.log(`   - Global Key Pool with ${(await scraperManager.apiKeys?.length) || '?'} keys`);
