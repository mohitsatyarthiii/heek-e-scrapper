...
const queueSchema = new mongoose.Schema({
  ...
  data: { 
    ...
    saveAllWithEmails: { type: Boolean, default: true } // Always save channels with emails
  },
        ...
        if (parts.length === 2) {
          ...
          if (domain.includes('.') && 
            ...
            if (localPart.length >= 1 && localPart.length <= 64 &&
              ...
              // Check if it's a business domain (not personal email domains)
              const businessDomains = [
                '.com', '.org', '.net', '.edu', '.gov', '.io', '.co', '.biz', '.info',
                '.uk', '.us', '.ca', '.au', '.de', '.fr', '.jp', '.cn', '.in', '.br',
                '.mx', '.it', '.es', '.nl', '.se', '.no', '.dk', '.fi', '.pl', '.ru',
                '.agency', '.company', '.business', '.enterprises', '.llc', '.inc',
                '.ltd', '.limited', '.corp', '.group', '.holdings', '.partners',
                '.associates', '.solutions', '.consulting', '.advocacy', '.network'
              ];
              
              const hasBusinessDomain = businessDomains.some(ext => domain.endsWith(ext));
              
              // For business emails, we're more lenient
              if (isBusinessEmail || hasBusinessDomain) {
                allEmails.add(cleaned);
              } 
              // For personal emails, we need to filter out common free email providers
              else {
                const freeEmailDomains = [
                  'gmail.com', 'yahoo.com', 'hotmail.com', 'outlook.com', 'aol.com',
                  'protonmail.com', 'mail.com', 'inbox.com', 'gmx.com', 'yandex.com',
                  'icloud.com', 'me.com', 'mac.com', 'live.com', 'msn.com', 'zoho.com',
                  'rediffmail.com', 'rocketmail.com', 'ymail.com', 'mail.ru', 'tutanota.com',
                  'fastmail.com', 'hushmail.com', 'runbox.com', 'countermail.com',
                  'gmx.us', 'gmx.net', 'web.de', 'online.de', 'freenet.de', 't-online.de',
                  'btinternet.com', 'virginmedia.com', 'ntlworld.com', 'blueyonder.co.uk',
                  'sky.com', 'talktalk.net', 'o2.co.uk', 'orange.fr', 'wanadoo.fr',
                  'free.fr', 'laposte.net', 'sfr.fr', 'neuf.fr', 'numericable.fr',
                  'tele2.fr', 'club-internet.fr', 'cegetel.net', 'noos.fr'
                ];
...
function extractPhoneNumbers(text) {
  ...
  return allPhones
    ...
    .filter(phone => {
      ...
      // Validate based on digit count and patterns
      ...
      if (/^(\d)\1{9,}$/.test(digits)) return false; // 1111111111 pattern
      
      // Check for sequential numbers
      if (/0123456789|1234567890/.test(digits)) return false;
      
      // Check if it starts with valid country codes or area codes
      const validStartPatterns = [
        /^[1-9]/, // Should not start with 0 (except for UK format)
        /^\+[1-9]/ // International should start with valid country code
      ];
      
      // If it has a plus, it must have valid country code
      if (phone.startsWith('+')) {
        const countryCode = digits.substring(0, 3);
        // Check if it's a common business country code
        const validCountryCodes = ['1', '44', '61', '91', '86', '49', '33', '81', '7', '55', '27'];
        const hasValidCode = validCountryCodes.some(code => countryCode.startsWith(code));
        if (!hasValidCode) return false;
      }
      
      // Additional checks for US/Canada numbers (most common)
      if (digitCount === 10 || (digitCount === 11 && digits.startsWith('1'))) {
        // Check area code (first 3 digits should be valid)
        const areaCode = digitCount === 10 ? digits.substring(0, 3) : digits.substring(1, 4);
        
        // Area code should not start with 0 or 1
        if (areaCode.startsWith('0') || areaCode.startsWith('1')) return false;
        
        // Area code should be a number between 200-999
        const areaCodeNum = parseInt(areaCode);
        if (areaCodeNum < 200 || areaCodeNum > 999) return false;
        
        // Check exchange code (next 3 digits)
        const exchangeCode = digitCount === 10 ? digits.substring(3, 6) : digits.substring(4, 7);
        
        // Exchange code should not start with 0 or 1 (except for special cases)
        if (exchangeCode.startsWith('0') || exchangeCode.startsWith('1')) return false;
      }
      
      // Check for UK format (starts with 0)
      if (phone.startsWith('0') && digits.length === 11) {
        // Valid UK area codes
        const ukAreaCodes = ['01', '02', '03', '07', '08'];
        const areaCode = digits.substring(0, 2);
        if (!ukAreaCodes.includes(areaCode)) return false;
      }
      
      return true;
    })
    .map(phone => {
      // Format for display (add + if it's an international number)
      if (phone.length === 10 && !phone.startsWith('+')) {
        ...
        // Assume it's an international number without plus
        return `+${phone}`;
      }
      return phone;
    });
...
async function processChannel(channelData, sourceInfo = {}, taskId = null, options = {}) {
  ...
  try {
    ...
    // If we have a website, scrape it for more contacts
    if (websiteUrl) {
    ...
    for (const bioLink of bioLinks) {
      try {
        ...
        const bioPhones = extractPhoneNumbers(bioText);
        const bioSocials = extractSocialLinks(bioText);
        
        emails = [...new Set([...emails, ...bioEmails])];
        phones = [...new Set([...phones, ...bioPhones])];
        socialLinks = [...new Set([...socialLinks, ...bioSocials])];
      } catch (e) {
        // Ignore errors
      }
    }
    ...
    // Determine if we should save - ALWAYS save if emails found
    const hasEmails = emails.length > 0;
    const shouldSave = hasEmails || 
                       phones.length > 0 || 
                       subscriberCount >= minSubscribers || 
                       qualityScore >= qualityThreshold;
    
    if (!shouldSave) {
      return { saved: false, skipped: true, reason: 'low_quality' };
    }
...
async function scrapeChannels(keywords, maxResults = 10000, taskId = null, options = {}) {
  ...
  await scraperLogger.info('🚀 Starting aggressive scrape', { 
  ...
  // Phase 2: Discover related channels (BFS)
  if (includeRelated && channelQueue.length > 0) {
    await scraperLogger.info('🔗 Discovering related channels', { 
      ...
      try {
        if (task.task === 'scrape_channels') {
          ...
          const stats = await scrapeChannels(keywords, count, task._id, {
            ...
            saveAllWithEmails: task.data.saveAllWithEmails !== false
          });
...
async function runDailyScrape() {
  await systemLogger.info('Starting daily aggressive scrape');
...
app.post('/api/scrape', async (req, res) => {
  try {
    ...
    await task.save();
    await apiLogger.success(`✅ New aggressive scrape task queued`, { 
      taskId: task._id, 
    ...
    res.json({ 
      message: 'Aggressive scraping task queued successfully', 
      taskId: task._id,
...
app.get('/api/stats', async (req, res) => {
  try {
    ...
    res.json({ 
      ...
      countryDistribution,
      sourceDistribution,
      saveRate: totalChannels > 0 ? ((channelsWithEmails + channelsWithPhones) / totalChannels * 100).toFixed(2) + '%' : '0%'
    });
  } catch (error) {
...
server.listen(PORT, () => {
  ...
  console.log(`   - ALWAYS saves channels with emails`);
  console.log(`   - Target: 10k+ channels and 5k+ emails per day`);
